...
# EXE  := tfswitch
# PKG  := github.com/skyzyx/tfswitch
# VER := $(shell git ls-remote --tags git@github.com:skyzyx/tfswitch.git | awk '{if ($$2 ~ "\\^\\{\\}$$") next; print vers[split($$2,vers,"\\/")]}' | sort -n -t. -k1,1 -k2,2 -k3,3 | tail -1)
# PATH := build:$(PATH)
